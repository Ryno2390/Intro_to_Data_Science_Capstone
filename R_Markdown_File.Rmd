...
Clinton_Moving_Average <- National_Polls %>%
  mutate(Clinton_MA = rollmean(rawpoll_clinton, 100, na.pad=TRUE, align="right"))

ggplot(data = Clinton_Moving_Average, aes(x=enddate, y=Clinton_MA)) +
  geom_line()
```
...
```{r}
Trump_Moving_Average <- National_Polls %>%
  mutate(Trump_MA = rollmean(rawpoll_trump, 100, na.pad=TRUE, align="right"))

ggplot(data = Trump_Moving_Average, aes(x=enddate, y=Trump_MA)) +
  geom_line()
```
